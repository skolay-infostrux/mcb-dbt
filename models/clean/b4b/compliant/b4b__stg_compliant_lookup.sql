{{ config(
    ...
    pre_hook="
        ...
        copy into {{ var('project_database_prefix') }}_{{ target.name }}_INGEST.{{ env_var('DBT_SNOWFLAKE_SCHEMA_PREFIX', '') }}B4B.RAW_LOOKUP(DATA_REC,FILE_ROW_NUMBER,FILE_NAME,FILE_ROW_KEY,FILE_INSERT_TS,MAX_ROW_NUMBER) 
            from (
                select 
                    $1  as DATA_REC
        ...
        PATTERN = '.*/B4_LOOK_.*.TXT.*';
        "  
    )
}}

{{ generate_stg_compliant_lookup('raw_data_b4b','raw_lookup') }}